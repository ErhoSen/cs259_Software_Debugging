...
def traceit(frame, event, trace_arg):
    ...
    if event == 'line':
        ...
        for watchpoint in watchpoints:
            if watchpoint in frame.f_locals:
                if watchpoint in watch_values:
                    if watch_values[watchpoint] != frame.f_locals[watchpoint]:
                        lines.append((watchpoint + " : " + repr(watch_values[watchpoint]) + " => " + repr(frame.f_locals[watchpoint])))
                else:
                    lines.append(watchpoint + " : Initialized => " + repr(frame.f_locals[watchpoint]))
                watch_values[watchpoint] = frame.f_locals[watchpoint]
        if len(lines) > 0:
